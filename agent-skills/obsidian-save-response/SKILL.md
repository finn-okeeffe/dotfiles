...
1. Discover the Obsidian MCP tools available in the current session. If they are unavailable, stop and tell the user that the Obsidian connection is required. Do not substitute direct filesystem access for note operations. The bundled image-copy script in step 6 is the sole filesystem exception.
...
6. Process attached media, if any, before creating the note:
   ...
   - For an image available to the agent as a local file, run `<skill-directory>/scripts/copy-image-to-vault.sh SOURCE_IMAGE DESTINATION_FILENAME`. Set `OBSIDIAN_VAULT_PATH` when the vault root is not `~/obsidian-vault`; the script always targets its `Attachments` child directory. Give `DESTINATION_FILENAME` a descriptive name. The script preserves the source extension, refuses to overwrite, prints the final absolute path, and adds a numeric suffix on collision. Do not recreate this copy logic manually.
   - For other local media, check whether the available Obsidian MCP exposes a binary-safe create/upload operation. If it does, place the file in `Attachments` through that operation. If it only exposes note writes and file moves, explain the limitation and ask the user to place the file in the vault or enable an upload-capable MCP operation. Do not create the note with broken embeds or claim the attachment was uploaded.
